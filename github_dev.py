...
# ---------------------------------------------------------------------------
# Papkani ichidagi barcha fayllari bilan o'chirish
# ---------------------------------------------------------------------------
_MAX_DELETE_FOLDER_FILES = 2000


def _clean_folder_path(path: str) -> str:
    """Papka yo'lini tekshiradi. Repository ildizini (root) o'chirishga yo'l qo'ymaydi."""
    parts = [p for p in (path or "").replace("\\", "/").split("/") if p]
    if not parts:
        raise GitHubDevError("Repository ildizini (root) bu usul bilan o'chirib bo'lmaydi.")
    if any(p in (".", "..") for p in parts):
        raise GitHubDevError("Papka yo'li noto'g'ri.")
    return "/".join(parts)


def _resolve_folder(repo: str, path: str, branch: str | None) -> dict[str, Any]:
    """Branch'ning HOZIRGI holatidan papkani topadi va shu papka ichidagi fayllarni qaytaradi.

    Faqat shu papkaning o'z subtree'i o'qiladi, shuning uchun natijada
    boshqa papka/fayllar bo'lishi mumkin emas.
    """
    path = _clean_folder_path(path)
    branch = branch or (get_repository(repo).get("default_branch") or "main")
    owner, name = _repo_parts(repo)
    ...
    commit = _request("GET", f"{_API}/repos/{owner}/{name}/git/commits/{head_sha}").json()
    root_tree = (commit.get("tree") or {}).get("sha")
    if not root_tree:
        raise GitHubDevError("Repository tree aniqlanmadi.")

    # Ildizdan boshlab papkagacha bosqichma-bosqich tushamiz.
    tree_sha = root_tree
    for segment in path.split("/"):
        level = _request("GET", f"{_API}/repos/{owner}/{name}/git/trees/{tree_sha}").json()
        match = next(
            (e for e in level.get("tree", []) if e.get("path") == segment and e.get("type") == "tree"),
            None,
        )
        if not match:
            raise GitHubDevError("Papka topilmadi (u allaqachon o'chirilgan bo'lishi mumkin).")
        tree_sha = match["sha"]

    listing = _request(
        "GET", f"{_API}/repos/{owner}/{name}/git/trees/{tree_sha}", params={"recursive": "1"}
    ).json()
    if listing.get("truncated"):
        raise GitHubDevError("Papka juda katta, GitHub uni to'liq ro'yxatlab bermadi. O'chirish bekor qilindi.")

    files = [e for e in listing.get("tree", []) if e.get("type") in ("blob", "commit")]
    return {
        "path": path,
        "branch": branch,
        "owner": owner,
...
def count_folder_files(repo: str, path: str, branch: str | None = None) -> int:
    """Papka ichidagi (ichki papkalar bilan) fayllar sonini qaytaradi. Hech narsani o'zgartirmaydi."""
    return len(_resolve_folder(repo, path, branch)["files"])


def delete_directory(repo: str, path: str, branch: str | None = None) -> dict[str, Any]:
    """Papkani ichidagi barcha fayllari bilan BITTA commit orqali o'chiradi.

    Faqat `path` ichidagi fayllar tree'dan olib tashlanadi (sha=None). Repository'ning
    boshqa barcha fayllari base_tree orqali o'z holicha qoladi.
    """
    info = _resolve_folder(repo, path, branch)
    path, branch = info["path"], info["branch"]
    owner, name = info["owner"], info["name"]
    files = info["files"]

    if not files:
        raise GitHubDevError("Bu papkada o'chiriladigan fayl topilmadi.")
    if len(files) > _MAX_DELETE_FOLDER_FILES:
        raise GitHubDevError(
            f"Papkada juda ko'p fayl bor: {len(files)} ta. Maksimum {_MAX_DELETE_FOLDER_FILES} ta."
        )

    prefix = path + "/"
    tree_entries = [
        {"path": prefix + e["path"], "mode": e["mode"], "type": e["type"], "sha": None}
        for e in files
    ]
    # Xavfsizlik: har bir yozuv aynan shu papka ichida bo'lishi shart.
    if not all(e["path"].startswith(prefix) for e in tree_entries):
        raise GitHubDevError("Ichki xato: papkadan tashqari yo'l aniqlandi. O'chirish bekor qilindi.")

    try:
        new_tree = _request(
            "POST",
            f"{_API}/repos/{owner}/{name}/git/trees",
            json={"base_tree": info["root_tree"], "tree": tree_entries},
        ).json()
    except GitHubDevError as exc:
        if any(_is_github_workflow_path(e["path"]) for e in tree_entries):
            raise GitHubDevError(
                "Papka ichida GitHub Actions workflow fayli bor. Uni o'chirish uchun tokenda "
                "Workflows = Read and write ruxsati kerak. "
    ...
    new_commit = _request(
        ...
        json={
            "message": f"Delete folder {path} ({len(files)} files)",
            "tree": new_tree_sha,
            "parents": [info["head_sha"]],
        },
    ).json()
    ...
    # force=False: agar branch orada o'zgarib ketgan bo'lsa, hech narsa ustidan yozilmaydi.
    try:
        _request(
            "PATCH",
        ...
        raise GitHubDevError(
            "Branch orada o'zgarib ketgani uchun papka o'chirilmadi. Hech narsa o'zgartirilmadi. "
            "Qayta urinib ko'ring."
        ) from exc

    logger.info(
        "GitHub folder deleted repo=%s branch=%s path=%s files=%d commit=%s",
        repo, branch, path, len(files), new_commit_sha,
    )
    return {"path": path, "branch": branch, "file_count": len(files), "commit_sha": new_commit_sha}
